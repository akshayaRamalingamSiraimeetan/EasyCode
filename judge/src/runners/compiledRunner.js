...
async function _runOne({ workspaceId, executableName, input }) {
  ...
  if (runResult.timedOut)            return { status: Status.TIME_LIMIT_EXCEEDED,   stdout: runResult.stdout, stderr: runResult.stderr };
  if (runResult.outputLimitExceeded) return { status: Status.OUTPUT_LIMIT_EXCEEDED, stdout: "",               stderr: "Output limit exceeded" };
  if (runResult.exitCode !== 0)      return { status: Status.RUNTIME_ERROR,         stdout: runResult.stdout, stderr: runResult.stderr };
